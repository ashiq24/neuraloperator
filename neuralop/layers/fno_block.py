...
class FNOBlocks(nn.Module):
            ...
            [
                skip_connection(
                    self.in_channels, self.out_channels, skip_type=fno_skip, n_dim=self.n_dim,\
                     kernel_size=kernel_size,output_scaling_factor=self.output_scaling_factor[i]
                )
    ...
    def forward(self, x, index=0, output_shape=None):
        ...
        if self.fno_skip == 'conv' and self.output_scaling_factor[index]<1:
        ...
        x_skip_fno = self.fno_skips[index](x_skip_fno_input)

        if self.convs.output_scaling_factor is not None and self.fno_skip!='conv':
            # x_skip_fno = resample(
            #     x_skip_fno,
